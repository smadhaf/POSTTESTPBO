...
class Makanan extends Barang {
    ...
    public Makanan(String nama, int stok, double harga, boolean makanan) {
    ...
    @Override
    public String toString() {
        return "Nama: " + super.getNama() + ", Stok: " + super.getStok() + ", Harga: " + super.getHarga() + ", makanan: " + makanan;
    }
}

class Amunisi extends Barang {
    private boolean amunisi;

    public Amunisi(String nama, int stok, double harga, boolean amunisi) {
        super(nama, stok, harga);
        this.amunisi = amunisi;
    }

    public boolean isamunisi() {
        return amunisi;
    }

    public void setamunisi(boolean amunisi) {
        this.amunisi = amunisi;
    }

    @Override
    public String toString() {
        return "Nama: " + super.getNama() + ", Stok: " + super.getStok() + ", Harga: " + super.getHarga() + ", Senjata Api: " + amunisi;
    }
}
public class InventoryProgram {
    ...
    private static void tambahBarang(ArrayList<Barang> inventory, Scanner scanner) {
        ...
        if (jenisBarang == 1) {
            ...
            tambahBarang(inventory, nama, stok, harga, isMakanan);
        } else if (jenisBarang == 2) {
            System.out.print("Apakah ini amunisi (Y/N)? ");
            scanner.nextLine();
            boolean isAmunisi = scanner.nextLine().equalsIgnoreCase("Y");
            tambahBarang(inventory, nama, stok, harga, isAmunisi);
        } else {
            System.out.println("Pilihan tidak valid.");
        }
    }
    // overload
    private static void tambahBarang(ArrayList<Barang> inventory, String nama, int stok, double harga, boolean isMakanan) {
        Barang barang;
        if (isMakanan) {
            barang = new Makanan(nama, stok, harga, isMakanan);
        } else {
            barang = new Amunisi(nama, stok, harga, isMakanan);
        }
